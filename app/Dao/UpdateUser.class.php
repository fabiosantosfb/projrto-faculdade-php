...
class UpdateUser extends Conexao {
	...
	public function upDocumento($nome, $usuario, $cpf, $rg, $dataexpedicao, $orgao_expedidor, $uf){
		try{
			$update = "UPDATE registro_database.pessoa_fisica SET cpf=:cpf, uf=:uf, rg=:rg, data_expedicao=:data, orgao_expedidor=:org WHERE id_usuario=:id";
			$validar = Parent::getInstanceConexao()->prepare($update);
			...
			if($validar->execute()){
				try{
					$update = "UPDATE registro_database.usuario SET nome=:nome WHERE id_usuario=:id";
					$validar = Parent::getInstanceConexao()->prepare($update);
	...
	public function upDocumentoPj($nome, $usuario, $cnpj){
		try{
			$update = "UPDATE registro_database.pessoa_juridica SET cnpj=:cnpj WHERE id_usuario=:id";
			$validar = Parent::getInstanceConexao()->prepare($update);
	...
	public function upAddress($cep, $cidade, $rua, $bairro, $numero, $complemento, $id_endereco){
		try{
			$update = "UPDATE registro_database.usuario SET cep=:cep, rua=:rua, bairro=:bairro, cidade=:cidade, numero=:numero, complemento=:complemento WHERE id_usuario=:id_endereco";
			$validar = Parent::getInstanceConexao()->prepare($update);
	...
	public function upPassword($pwd, $email, $id){
		...
		try{
			$update = "UPDATE registro_database.usuario SET email=:email, senha=:senha WHERE id_usuario=:id";
			$validar = Parent::getInstanceConexao()->prepare($update);
	...
	public function redefinirPassword($pwd, $email){
		...
		try{
			$update = "UPDATE registro_database.usuario SET senha=:pwd WHERE email=:_email";
			$validar = Parent::getInstanceConexao()->prepare($update);
